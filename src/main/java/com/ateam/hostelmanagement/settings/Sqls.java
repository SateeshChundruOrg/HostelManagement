...
//    public static String SELECT_ROOMS="select * from room WHERE deleted=0";
    public static String SELECT_ROOMS="select 	r.roomId, r.roomNumber, r.noOfBeds,r.deleted, r.hostelId,h.hostelName from room r join hostel h on(r.hostelId=h.hostelId)";
   ...
   // public static String INSERT_PAYMENT="insert into Payments (paymentId,amount,actualDate,paidDate,hostlerId) values (?,?,?,?,?)";
    ...
    public static String SELECT_ROOM_BEDS_AVAILABLE="select roomId,roomNumber,r.deleted,r.hostelId,noOfBeds,h.hostelName,(noOfBeds-(select count(roomId) as filledcount from RoomHostlerMapping as rhmap where r.deleted=0 and rhmap.roomId=r.roomId)) as availableBeds from room r join hostel h on(r.hostelId=h.hostelId)";
